...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn review(input: &'a PendingToolReviewState) -> Self {
        Self {
            event_name: "tool review",
            thread_id: &input.thread_id,
            turn_id: Some(&input.turn_id),
            review_id: Some(&input.review_id),
...
impl AnalyticsReducer {
    ...
    fn ingest_server_response(
        ...
        match response {
            ...
            ServerResponse::PermissionsRequestApproval {
                ...
                let (status, resolution) = match response.scope {
                    PermissionGrantScope::Turn => (ReviewStatus::Approved, ReviewResolution::None),
    ...
    fn emit_tool_review_event(
        ...
        if let Some(item_id) = pending_review.item_id.as_ref() {
            self.record_tool_review_summary(item_id, reviewer, status, resolution, &pending_review);
        }
        let Some((connection_state, thread_metadata)) =
    ...
    fn record_tool_review_summary(
        ...
        summary.final_approval_outcome = Some(tool_item_final_approval_outcome(
            reviewer, status, resolution,
        ));
...
fn tool_item_final_approval_outcome(
    ...
    match (reviewer, status, resolution) {
        (Reviewer::Guardian, ReviewStatus::Approved, _) => {
            ToolItemFinalApprovalOutcome::GuardianApproved
        }
        (Reviewer::Guardian, ReviewStatus::Denied, _) => {
            ToolItemFinalApprovalOutcome::GuardianDenied
        }
        (Reviewer::Guardian, _, _) => ToolItemFinalApprovalOutcome::GuardianAborted,
        (Reviewer::User, ReviewStatus::Approved, ReviewResolution::SessionApproval) => {
            ToolItemFinalApprovalOutcome::UserApprovedForSession
        }
        (Reviewer::User, ReviewStatus::Approved, _) => ToolItemFinalApprovalOutcome::UserApproved,
        (Reviewer::User, ReviewStatus::Denied, _) => ToolItemFinalApprovalOutcome::UserDenied,
        (Reviewer::User, _, _) => ToolItemFinalApprovalOutcome::UserAborted,
    }
}
