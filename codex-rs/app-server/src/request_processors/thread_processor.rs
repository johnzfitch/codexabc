...
fn thread_read_view_error(err: ThreadReadViewError) -> JSONRPCErrorError {
    match err {
        ...
        ThreadReadViewError::Unsupported(message) => method_not_found(message),
...
fn thread_turns_list_history_load_error(
    ...
    match err {
        ...
        ThreadStoreError::Unsupported { operation } => {
            ThreadReadViewError::Unsupported(format!("{operation} is not supported yet"))
        }
...
fn thread_store_archive_error(operation: &str, err: ThreadStoreError) -> JSONRPCErrorError {
    match err {
        ...
        } => method_not_found(format!("{unsupported_operation} is not supported yet")),
